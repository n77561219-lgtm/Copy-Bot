...
_FORMAT_HINTS = {
    "экспертный пост": (
        "Структура: сильный тезис → аргументы/доказательства → вывод. "
        "Демонстрируй экспертизу фактами, цифрами или опытом. Без воды."
    ),
    "кейс": (
        "Структура: ситуация (было) → действия (что сделали) → результат (стало). "
        "Конкретные цифры и детали обязательны. Вывод — чему учит этот кейс."
    ),
    "сторителлинг": (
        "Структура: завязка (вовлечение) → развитие (конфликт/напряжение) → развязка (урок/мораль). "
        "Пиши как историю от первого лица, с деталями и эмоцией."
    ),
    "провокация / горячее мнение": (
        "Структура: спорный тезис (режет слух) → почему ты так думаешь → вызов читателю. "
        "Не бойся быть неудобным. Цель — вызвать реакцию и дискуссию."
    ),
    "продающий пост": (
        "Структура: боль/проблема → решение → выгоды → призыв к действию. "
        "Фокус на пользе для читателя, не на характеристиках. CTA в конце — чёткий и конкретный."
    ),
    "лайфхак": (
        "Структура: проблема/ситуация → конкретный приём → результат. "
        "Максимально практично, без теории. Читатель должен сразу понять, как применить."
    ),
    "личный опыт": (
        "Структура: что произошло → что я понял/почувствовал → как это изменило подход. "
        "Честно, уязвимо, без глянца. Личные детали делают пост живым."
    ),
    "обучающий пост": (
        "Структура: что объясняем → шаги/принципы (2-4 пункта) → итог/применение. "
        "Простой язык, конкретные примеры. Читатель уходит с новым знанием."
    ),
    "рилс-сценарий": (
        "Структура строго: \n"
        "1. ХУКОВАЯ ФРАЗА (1-2 предложения — то, что говорят в первые 2 секунды видео)\n"
        "2. ТЕКСТ ЗА КАДРОМ / субтитры (основное содержание, короткими репликами)\n"
        "3. ПРИЗЫВ В КОНЦЕ (что сделать после просмотра)\n"
        "Каждая реплика — отдельная строка. Стиль разговорный, динамичный."
    ),
    "новость с комментарием": (
        "Структура: ФАКТ (что случилось, 1-3 предложения, со ссылкой на источник если есть) → "
        "АВТОРСКИЙ КОММЕНТАРИЙ (почему это важно, что думаешь, какой вывод для читателя). "
        "Чётко разделяй факт и мнение."
    ),
}
...
async def run_copywriter(
    ...
    if post_type:
        hint = _FORMAT_HINTS.get(post_type.lower(), "")
        if hint:
